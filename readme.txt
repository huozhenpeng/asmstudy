...
一点思考：
     ...
     这本身是一种AOP思想，可以做日志打印、耗时统计、权限管理（比如需要先登录，跳转登录页面）



继续优化

    实现目的：可以在build.gradle中配置耗时时间，超过这个时间的就统计，否则就不统计

    beforeEvaluate  afterEvaluate含义
...
笔记
    ...
    ASMUnitTest2类：之前写的输出语句

    ASMUnitTest类：添加了判断语句，这样就可以改进，比如说当耗时大于200ms时才输出